    ...
    <div class="vstack align-top border rounded p-2 text-nowrap" style="overflow-x:auto">
        <div class="hstack">
            <span class="input-group-text py-0">
                <i class="bi bi-arrow-left-right" style="cursor:pointer" @click="toggleAccountList"></i>
            </span>
            <i class="bi bi-wallet-fill text-primary px-1"></i>
                <span v-if="selectedAccountName" class="fst-normal text-danger">[{{this.$props.selectedAccount.id}}]</span>
            </i>
            <span v-if="selectedAccountName" class="col-auto">{{selectedAccountName}}</span>
            <span v-else class="col-auto">Account : None</span>
            <input class="ms-auto me-1" type="checkbox" v-model="viewSimple"> View only that have cost.
        </div>

        <div v-if="this.$props.pmode == 'execution'" class="hstack">
            <div class="col mt-2 mb-1 text-start">
                <div class="col input-group" style="width:570px;">
                    <lable class="input-group-text py-0">Year</lable>
                    <select v-model="targetYear" class="form-select p-0 ps-2"
                        @change="loadData" :disabled="this.$props.pmode == 'plan' && this.$props.acl < 9">
                    ...
                    <div class="form-control border-0 border-start" style="padding:0.1em 0.5em; width:200px;">
                        <input type="checkbox" class="align-middle" v-model="checkChildOrganization" @change="loadData"> Include Children Organization
                    </div>
                </div>
            </div>

            <div v-if="selectedAccountName" class="col-auto ms-auto mb-1">
        ...
        <div v-else>
            <div class="col mt-2 mb-1 text-start">
                <div class="col input-group" style="width:400px;">
                    <lable class="input-group-text py-0">Year</lable>
                    <select v-model="targetYear" class="form-select p-0 ps-2"
                        @change="loadData" :disabled="this.$props.pmode == 'plan' && this.$props.acl < 9">
                        <option :value="this.$props.year -2">{{this.$props.year -2}}</option>
        ...
        <div v-else-if="this.$props.pmode == 'plan'" class="table-responsive">
            <table class="table table-hover table-bordered">
                <thead style="background-color:#DDEEFF;position: sticky;top:0;" >
                    <tr>
                        <th colspan="1" class="py-1 text-center align-middle">Organization</th>
                    ...
                    <tr>
                        ...
                        <th class="py-1 text-center">{{targetYear + 1}} Year</th>
                    </tr>
                </thead>
                <tbody v-for="item,index in itemList" class="shadow-sm">
                    <tr v-if="isView(item)" class="bg-opacity-25"
                        ...
                        <td class="hstack border-0 border-start">
                            ...
                            <span style="cursor:pointer;" @click="viewBudgetList(item)">{{ item.org_name }}</span>
...
<script>
export default {
    props : ['site','lang','pmode','selectedAccount','year','month','acl','div_name'],
    components: {
    ...
    data(){
        return {
            ...
            target_div_name : this.$props.div_name,
    ...
    methods: {
        ...
        async loadData(event,selectedItemId=''){
            ...
            if(this.$props.selectedAccount.id == undefined){
                this.itemList = [];
                return;
            }
            ...
            await axios.get('./api/getBudgetAmount',
                    { params:{  mode : vm.$props.pmode,
                                account_id: vm.$props.selectedAccount.id,
                                target_year : vm.targetYear,
                                target_div : vm.target_div_name,
        ...
        viewBudgetList(item,target='',month=''){
            ...
            // changeMonth 로 data 는 변경되었으나, selectedColumn 이 년간 항목일 경우 emit 하지 않음
            if(this.$props.selectedAccount == this.selectedAccount && this.selectedItem.id == item.id
                && (target == '' && this.selectedColumn.includes("year")) )
                return;
            ...
            if(this.selectedColumn.includes("year")){
                startMonth = 1;
                endMonth = 12;
            }
            this.$emit('select-bfa-item', item, this.$props.selectedAccount,
                    this.selectedColumn, startMonth, endMonth, this.checkChildOrganization);
        },
        toggleAccountList(){
            this.$emit('toggle-accountlist');
        },
        enableFilter(){
    ...
    computed:{
        ...
        selectedAccountName(){
            if(this.$props.selectedAccount)
                return this.$props.selectedAccount.account_name;
            return "";
        },
        markedCost(){
            return (val) => {
