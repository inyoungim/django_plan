        ...
        <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
            <div class="modal-content text-center">
                <div class="modal-header py-2 bg-success text-white" style="cursor:move">
                    <h4 class="col">Part Number Search</h4>
                    <button type="button" class="btn-close btn-close btn-close-white" @click="closeSearchModal"></button>
                </div>

                <div class="modal-body" style="min-height:150px; max-height:450px;">
                    ...
                    <table class="table table-hover table-bordered">
                        <thead style="background-color:#DDEEFF;">
                            <tr>
                                <th class="py-1 text-center">Part Number</th>
                                <th class="py-1 text-center">Description</th>
                                <th class="py-1 text-center">Specification</th>
                         ...
                         <tbody class="shadow-sm">
                            <tr v-for="(data,i) in tdata.searchList" @click="setTarget(data)" style="cursor:pointer">
                                <td class="py-1 text-start">{{data.part_number}}</td>
                                <td class="py-1 text-start">{{data.description}}</td>
                                <td class="py-1 text-start"><span v-html="data.sepecification"></span></td>
                                <td class="py-1 text-center">{{data.div_code}}</td>
                                <td class="py-1 text-center">{{data.period_name}}</td>
                                <td class="py-1 text-end">{{ markedCost(data.part_cost) }}</td>
                            </tr>
...
export default {
    ...
    methods: {
        ...
        async page(num, page_num, event){
            ...
            var vm = this;
            await axios.get('./api/partSearch',
                { params:{ active_page: num, page_num: page_num,
                           option : vm.option, search_text : vm.search_string, } })
                .then(function(res){
                    vm.tdata = res.data;
