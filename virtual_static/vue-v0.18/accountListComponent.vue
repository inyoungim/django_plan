    ...
    <div class="vstack align-top border rounded p-2 text-nowrap" style="overflow-x:auto">
        <div class="col-auto">
            <span class="input-group-text p-1">
                ...
                <div class="px-1">Budget Account</div>
        ...
        <div class="m-0 p-0">
            ...
            <div v-for="item,index in itemList">
                <div v-if="isView(item) && (this.$props.pmode == 'mgmt' || item.new_enable)"
                     :id="item.id" class="hstack align-middle p-1 bg-opacity-50"
                     :class="{'bg-success': selectedItem.id == item.id}"
                     style="cursor:pointer" @click="selectItem(item)" @dblclick="this.$emit('dblclick-item');">
                    ...
                    <div class="hstack">
                        <i v-if="!item.new_enable" class="bi bi-shield-x text-danger me-1"></i>

                        <span v-if="toggle_id2" class="text-danger">[{{ item.id }}]</span>
...
export default {
    ...
    methods: {
        ...
        async loadData(event,selectedItemId=''){
            ...
            var vm = this;
            await axios.get('./api/getAccountList', { params:{ div_name: vm.$props.div_name ,} })
                .then(function(res){
