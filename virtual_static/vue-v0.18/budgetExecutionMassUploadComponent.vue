            ...
            <div class="modal-content text-center">
                        ...
                        <tbody class="shadow-sm">
                            <tr v-for="acc in accountItemList" class="py-1 text-center">
                                <td v-for="(key,i) in accountKeyList" class="py-1 text-center text-nowrap bg-opacity-25"
                                    ...
                                    <span :class="{'text-danger' : (!acc.org_name_check && i == 2) || (!acc.project_code_check && i == 3) }">
                                        <span v-if="key == 'org_name' && acc[key] != acc['input_org_name']">{{acc['input_org_name']}} --> {{acc[key]}}</span>
                                        <span v-else>{{acc[key]}}</span>
                                    </span>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="modal-footer">
                    <div class="col text-center">
                        ...
                        <div class="btn btn-secondary mx-2" style="width:100px;" @click="closeModal">Cancel</div>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    props : ['site','lang','year','month','acl','div_name','pmode'],
    components: {
    ...
    methods: {
        ...
        async downloadTemplate(){
            ...
            await axios.get('./api/massExecutionExcelDownload/'+this.accountItem.id,
                { params:{ 'mode': 'check' } } )
                .then(function(res){  })
                .catch(function(err){
            ...
            this.showProgressModal = true;
            await axios.get('./api/massExecutionExcelDownload/'+this.accountItem.id, config )
                .then(function (res) {
            ...
            await axios.delete('./api/massExecutionExcelDownload/'+this.accountItem.id, null )
                .then(function (res) { })
                .catch(function (err){
                    warningModal.show_modal(err);
                    return;
        ...
        async uploadStart(event){
            ...
            await axios.post('./api/massExecutionExcelUpload/'+this.accountItem.id,response )
                .then(function(res){
        ...
        applyData(){
            for(var i=0; i<this.accountItemList.length; i++){
                if(!this.accountItemList[i].validation_check){
                    warningModal.show_modal("Please, try again after correct data.");
                    return;
                }
            }
            this.$emit('mass-add-each-item', this.accountItemList ); //AddEachItem
            this.showModal = false;
        },
    },
}
