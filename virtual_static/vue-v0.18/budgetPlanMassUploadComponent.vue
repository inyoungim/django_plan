        ...
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content text-center">
                <div class="modal-header py-2 bg-success text-white" style="cursor:move">
                    <h4 v-if="lang=='kr'" class="col">일괄 업로드</h4><h4 v-else class="col">Mass Upload</h4>
                        ...
                        <tbody class="shadow-sm">
                            <tr v-for="d in dataItemList" class="py-1 text-center">
                                <td v-for="(key,i) in dataKeyList" class="py-1 text-start text-nowrap bg-opacity-25"
                                    :class="{'bg-danger': !d.validation_check}">
                                    <span :class="{'text-danger' : ( !d.nas_code_check && i == 3 ||
                                            !d.org_name_check && i == 4 || !d.account_id_check && i == 5
                                            || !d.account_name_check && i == 7
                                        ) }">
                                        {{d[key]}}
                                    </span>
                                </td>
...
export default {
    props : ['site','lang','acl','div_name','year','version'],
    components: {
    ...
    methods: {
        ...
        async uploadStart(event){
            ...
            await axios.post('./api/massPlanExcelUpload', response )
                .then(function(res){
        ...
        async applyData(){
            ...
            await axios.put('./api/massPlanExcelUpload',data )
                .then(function(res){ })
                .catch(function(err){
                    warningModal.show_modal(err);
                    return;
