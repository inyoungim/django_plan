        ...
        <div class="modal-dialog modal-dialog-centered modal-xxl modal-dialog-scrollable">
            <div class="modal-content text-center">
                <div class="modal-header py-2 bg-success text-white" style="cursor:move">
                    <h4 class="col">Account Execution Option Column Master</h4>
                ...
                <div class="modal-body" style="min-height:150px;">
                    ...
                    <table class="table table-hover table-bordered">
                        <thead style="background-color:#DDEEFF;">
                            <tr>
                                <th v-if="this.$props.pmode == 'mgmt' && this.$props.acl >= 7" class="py-1 text-center">
                                ...
                                <th class="py-1 text-start">Column Data</th>
                                <th class="py-1 text-start">Column Description</th>
                            </tr>
                        </thead>
                        <tbody v-if="this.$props.pmode == 'mgmt' && this.$props.acl >= 7" class="shadow-sm">
                        ...
                        <tbody v-else class="shadow-sm">
                            <tr v-for="data in itemList" class="bg-opacity-25"
                                style="cursor:pointer" :class="{'bg-warning': selectedItem.id == data.id}"
                                @click="selectedItem=data" @dblclick="checkDblClick(data)">
                                <td class="py-1 text-center">{{data.order_no}}</td>
                                <td class="py-1 text-start">{{data.column_name}}</td>
                                <td class="py-1 text-start">{{data.column_interface}}</td>
                                <td class="py-1 text-start">{{data.column_data}}</td>
...
export default {
    ...
    methods: {
        ...
        async searchData(event){
            ...
            var vm = this;
            await axios.get('./api/executionOptionColumnMaster/0',
        ...
        async submitFormModal(mode=null){
            ...
            await axios.post('./api/executionOptionColumnMaster/0', vm.targetItem)
                .then(function(res){
                    vm.itemList = res.data.itemList;
                    vm.mode = "list";
                })
        ...
        async updateFormModal(){
            ...
            var vm = this;
            vm.targetItem.search_string = vm.search_string;
            await axios.put('./api/executionOptionColumnMaster/'+vm.targetItem.id, vm.targetItem)
                .then(function(res){
                    vm.itemList = res.data.itemList;
                    vm.mode = "list";
                })
        ...
        async modal_ok(){
            ...
            var vm = this;
            if(vm.checked_ids[0]){
                await axios.delete('./api/executionOptionColumnMaster/'+vm.checked_ids[0],
                    { params:{ search_string : vm.search_string } }
                    )
                    .then(function(res){
                        vm.itemList = res.data.itemList;
        ...
        async moveUpColumn(){
            ...
            vm.targetItem.search_string = vm.search_string;
            await axios.put('./api/executionOptionColumnMaster/0', vm.targetItem)
                .then(function(res){
                    vm.itemList = res.data.itemList;
                    vm.mode = "list";
                })
